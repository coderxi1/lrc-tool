...
function biMultiplyDigit(a, b) {
  var c, d, e, f
  for (var result = new BigInt(), c = biHighIndex(a), d = 0, f = 0; c >= f; ++f) (e = result.digits[f] + a.digits[f] * b + d), (result.digits[f] = e & maxDigitVal), (d = e >>> biRadixBits)
...
function getNeteaseEncrypt(data) : any {
  return asrsea(data, e, f, g)
}

function encrypt(params: any): { toURLSearchParams: () => URLSearchParams } {
  const encrypted = getNeteaseEncrypt(JSON.stringify(params))
  encrypted.toURLSearchParams = () =>
    new URLSearchParams({
