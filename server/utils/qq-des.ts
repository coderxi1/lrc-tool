...
function BITNUM(a: number[], b: number, c: number): number {
  return (((a[flr(b / 32) * 4 + 3 - flr((b % 32) / 8)] >>> (7 - (b % 8))) & 0x01) << c) >>> 0
}
...
function InvIP(state: number[], in_: number[]) {
  in_[3] = BITNUMINTR(state[1], 7, 7) | BITNUMINTR(state[0], 7, 6) | BITNUMINTR(state[1], 15, 5) | BITNUMINTR(state[0], 15, 4) | BITNUMINTR(state[1], 23, 3) | BITNUMINTR(state[0], 23, 2) | BITNUMINTR(state[1], 31, 1) | BITNUMINTR(state[0], 31, 0)

  in_[2] = BITNUMINTR(state[1], 6, 7) | BITNUMINTR(state[0], 6, 6) | BITNUMINTR(state[1], 14, 5) | BITNUMINTR(state[0], 14, 4) | BITNUMINTR(state[1], 22, 3) | BITNUMINTR(state[0], 22, 2) | BITNUMINTR(state[1], 30, 1) | BITNUMINTR(state[0], 30, 0)

  in_[1] = BITNUMINTR(state[1], 5, 7) | BITNUMINTR(state[0], 5, 6) | BITNUMINTR(state[1], 13, 5) | BITNUMINTR(state[0], 13, 4) | BITNUMINTR(state[1], 21, 3) | BITNUMINTR(state[0], 21, 2) | BITNUMINTR(state[1], 29, 1) | BITNUMINTR(state[0], 29, 0)

  in_[0] = BITNUMINTR(state[1], 4, 7) | BITNUMINTR(state[0], 4, 6) | BITNUMINTR(state[1], 12, 5) | BITNUMINTR(state[0], 12, 4) | BITNUMINTR(state[1], 20, 3) | BITNUMINTR(state[0], 20, 2) | BITNUMINTR(state[1], 28, 1) | BITNUMINTR(state[0], 28, 0)

  in_[7] = BITNUMINTR(state[1], 3, 7) | BITNUMINTR(state[0], 3, 6) | BITNUMINTR(state[1], 11, 5) | BITNUMINTR(state[0], 11, 4) | BITNUMINTR(state[1], 19, 3) | BITNUMINTR(state[0], 19, 2) | BITNUMINTR(state[1], 27, 1) | BITNUMINTR(state[0], 27, 0)

  in_[6] = BITNUMINTR(state[1], 2, 7) | BITNUMINTR(state[0], 2, 6) | BITNUMINTR(state[1], 10, 5) | BITNUMINTR(state[0], 10, 4) | BITNUMINTR(state[1], 18, 3) | BITNUMINTR(state[0], 18, 2) | BITNUMINTR(state[1], 26, 1) | BITNUMINTR(state[0], 26, 0)

  in_[5] = BITNUMINTR(state[1], 1, 7) | BITNUMINTR(state[0], 1, 6) | BITNUMINTR(state[1], 9, 5) | BITNUMINTR(state[0], 9, 4) | BITNUMINTR(state[1], 17, 3) | BITNUMINTR(state[0], 17, 2) | BITNUMINTR(state[1], 25, 1) | BITNUMINTR(state[0], 25, 0)

  in_[4] = BITNUMINTR(state[1], 0, 7) | BITNUMINTR(state[0], 0, 6) | BITNUMINTR(state[1], 8, 5) | BITNUMINTR(state[0], 8, 4) | BITNUMINTR(state[1], 16, 3) | BITNUMINTR(state[0], 16, 2) | BITNUMINTR(state[1], 24, 1) | BITNUMINTR(state[0], 24, 0)
}
...
function des_key_setup(key: number[], schedule: number_0_6, mode: DES_MODE) {
  ...
  const key_rnd_shift = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1] as const
  const key_perm_c = [56, 48, 40, 32, 24, 16, 8, 0, 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35] as const
  const key_perm_d = [62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 60, 52, 44, 36, 28, 20, 12, 4, 27, 19, 11, 3] as const
  const key_compression = [13, 16, 10, 23, 0, 4, 2, 27, 14, 5, 20, 9, 22, 18, 11, 3, 25, 7, 15, 6, 26, 19, 12, 1, 40, 51, 30, 36, 46, 54, 29, 39, 50, 44, 32, 47, 43, 48, 38, 55, 33, 52, 45, 41, 49, 35, 28, 31] as const
  ...
  for (i = 0; i < 16; ++i) {
    ...
    for (j = 0; j < 24; ++j) {
      schedule[to_gen][flr(j / 8)] |= BITNUMINTR(C, key_compression[j], 7 - (j % 8))
      schedule[to_gen][flr(j / 8)] >>>= 0
    }
    for (; j < 48; ++j) {
      schedule[to_gen][flr(j / 8)] |= BITNUMINTR(D, key_compression[j] - 27, 7 - (j % 8))
      schedule[to_gen][flr(j / 8)] >>>= 0
    }
  }
}
...
export function des(buff: number[], key: number[], len: number): number {
  ...
  for (let i = 0; i < len; i += 8) des_crypt(ofst(buff, i), ofst(buff, i), schedule)
  return 0
}
...
export const k1 = [...'!@#)(NHLiuy*$%^&'].map((c) => c.codePointAt(0) as number)
export const k2 = [...'123ZXC!@#)(*$%^&'].map((c) => c.codePointAt(0) as number)
export const k3 = [...'!@#)(*$%^&abcDEF'].map((c) => c.codePointAt(0) as number)

import zlib from "zlib";
export async function decodeQRC(hexString: string) {
